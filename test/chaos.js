...
test('chaos - recovers after random disconnections (takes ~60s)', { timeout: 60000 }, async t => {
  ...
  for (const [swarm, expectedPeers] of peersBySwarm) {
    t.is(swarm.connections.size, expectedPeers.size, 'swarm has the correct number of connections')
    const missingKeys = []
    for (const conn of swarm.connections) {
      const key = conn.remotePublicKey.toString('hex')
      if (!expectedPeers.has(key)) missingKeys.push(key)
    }
    t.is(missingKeys.length, 0, 'swarm is not missing any expected peers')
  }

  await destroy(...swarms)
})
